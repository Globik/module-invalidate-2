//module-invalidate-2.js
'use strict';
//it's a fork from module-invalidate.js
// see original on https://github.com/FranckFreiburger/module-invalidate
...
const debug = require('debug')('module-invalidate-2');
...
function createProxy(mod) {
	return new Proxy(function() {}, {
		...
		getPrototypeOf: function(target) {
			...
			mod._exports === invalid && reload(mod, 'getPrototypeOf');
			return Reflect.getPrototypeOf(mod._exports);
		},
		
		setPrototypeOf: function(target, prototype) {
			
			mod._exports === invalid && reload(mod, 'setPrototypeOf');
			return Reflect.setPrototypeOf(mod._exports, prototype);
		},
		
		isExtensible: function(target) {
			
			mod._exports === invalid && reload(mod, 'isExtensible');
			return Reflect.isExtensible(mod._exports);
		},
		
		preventExtensions: function(target) {
			
			mod._exports === invalid && reload(mod, 'preventExtensions');
			return Reflect.preventExtensions(mod._exports);
		},
		
		getOwnPropertyDescriptor: function(target, prop) {
			
			mod._exports === invalid && reload(mod, 'getOwnPropertyDescriptor');
		...
		defineProperty: function(target, property, descriptor) {
			...
			mod._exports === invalid && reload(mod, 'defineProperty');
			return Reflect.defineProperty(mod._exports, property, descriptor);
		},
		
		has: function(target, prop) {
			
			mod._exports === invalid && reload(mod, 'has');
			return Reflect.has(mod._exports, prop);
		},
		
		get: function(target, property) {
			
			mod._exports === invalid && reload(mod, 'get');
		...
		set: function(target, property, value) {
			...
			mod._exports === invalid && reload(mod, 'set');
			return Reflect.set(mod._exports, property, value);
		},
		
		deleteProperty: function(target, property) {
			
			mod._exports === invalid && reload(mod, 'deleteProperty');
			return Reflect.deleteProperty(mod._exports, property);
		},
		
		ownKeys: function(target) {
			
			mod._exports === invalid && reload(mod, 'ownKeys');
		...
		apply: function(target, thisArg, argumentsList) {
			...
			mod._exports === invalid && reload(mod, 'apply');
			return Reflect.apply(mod._exports, thisArg, argumentsList);
		},
		
		construct: function(target, argumentsList, newTarget) {
			
			mod._exports === invalid && reload(mod, 'construct');
...
// I'm very sorry but I can't accept it. If you have a directory with above 100 javascript modules
// you wouldn't be so excited to write to all of them this  flag: 'module.invalidable=true | false'
// in a development mode this flag by default should be always set to true
...
Object.defineProperty(Module.prototype, 'invalidable', {
	...
	set: function(value) {
		...
		if ( this._proxy ) {
			...
			if ( value )console.log('creating a proxy')
...
Object.defineProperty(Module.prototype, 'exports', {
	get: function() {
// sooo why not simply return the proxy and "Teufel damit"?
